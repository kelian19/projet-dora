...
Chaîne complète, calibrée sur la base PRC 2025 réelle, convertie via Jacobs :
  - Remédiation : PRC 2025 + Jacobs (b=0.76) -> xi_effectif = 0.988.
                  Fréquence Binomiale Négative (lambda=2.0, facteur=9.2).
...
---------------------------
À queue lourde (xi_effectif proche ou > 1), le SCR n'est PAS déterminé par 
les données mais par l'HYPOTHESE DE PERTE MAXIMALE (le plafond individuel). 
Le capital croît linéairement avec le plafond. On ne "mesure" donc pas un 
SCR ponctuel ; on le conditionne à un jugement de perte maximale réaliste.

CHOIX CENTRAL : plafond = 40 M€, justifié par la capacité de réassurance cyber
mobilisable sur le marché (~5% du CA de l'entité). 
"""
...
def main():
    ...
    print("=" * 74)
    print(" SCR_DORA — SCENARIO CENTRAL (Plafond 40 M€, Jacobs b=0.76, Theta=1.5)")
    print("=" * 74)
    ...
    print(f"   {'L_DORA (Gumbel Theta=1.5)':30s} VaR99.5 = {scr_central/1e6:7.2f} M€  <- REFERENCE")
    ...
    # --- Sensibilité à xi effectif (Jacobs) ---
    print("\n" + "=" * 74)
    print(" SENSIBILITE A xi EFFECTIF (Pente de conversion Jacobs b)")
    print("=" * 74)
    ...
    print("=" * 74)
    print("""   - A xi proche de 1, l'espérance de la queue diverge. Le SCR est
     gouverné par le plafond (perte max réaliste = 40 M€ justifiés par
     la capacité de réassurance). Il se CONDITIONNE, il ne se mesure pas.
   - Fréquence : Binomiale négative (facteur 9.2) calibrée sur PRC 2025.
   - L'écart avec la formule standard démontre l'apport du modèle interne.
   - Le modèle est vectorisé et s'exécute en < 1 seconde.""")
    print("=" * 74)
